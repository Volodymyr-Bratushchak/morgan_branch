/**
 * @file
 * Contains \Drupal\person_entity\Plugin\Search\ContactSearch.
 */

namespace Drupal\person_entity\Plugin\Search;
...
/**
 * Handles searching for content_entity_example_contact entities.
 *
 ...
 *   content_entity_example/Plugin/Search.
 ...
 *
 * @SearchPlugin(
 *   id = "content_entity_example_contact_search",
 *   title = @Translation("Content Entity Example Contacts")
 * )
 */
class ContactSearch extends ConfigurableSearchPluginBase implements AccessibleInterface, SearchIndexingInterface {
  ...
  public function __construct(array $configuration, $plugin_id, $plugin_definition, Connection $database, EntityManagerInterface $entity_manager, ModuleHandlerInterface $module_handler, Config $search_settings, LanguageManagerInterface $language_manager, RendererInterface $renderer, AccountInterface $account = NULL) {
    ...
    $this->addCacheTags(['contact_list']);
  }
  ...
  protected function prepareResults(StatementInterface $found) {
    ...
    $entity_storage = $this->entityManager->getStorage('content_entity_example_contact');
    $entity_render = $this->entityManager->getViewBuilder('content_entity_example_contact');
    $keys = $this->keywords;
    ...
    foreach ($found as $item) {
      ...
      $rendered .= ' ' . $this->moduleHandler->invokeAll('contact_update_index', [$entity]);

      $extra = $this->moduleHandler->invokeAll('contact_search_result', [$entity]);
   ...
   */
  protected function addContactRankings(SelectExtender $query) {
  ...
  public function updateIndex() {
    ...
    $result = $this->database->queryRange("SELECT c.id, MAX(sd.reindex) FROM {contact} c LEFT JOIN {search_dataset} sd ON sd.sid = c.id AND sd.type = :type WHERE sd.sid IS NULL OR sd.reindex <> 0 GROUP BY c.id ORDER BY MAX(sd.reindex) is null DESC, MAX(sd.reindex) ASC, c.id ASC", 0, $limit, array(':type' => $this->getPluginId()), array('target' => 'replica'));
   ...
   */
  protected function indexContact(ContactInterface $entity) {
    ...
    foreach ($languages as $language) {
      ...
      $extra = $this->moduleHandler->invokeAll('contact_update_index', [$entity]);
      foreach ($extra as $t) {
        $text .= $t;
      }
  ...
  public function indexStatus() {
    ...
    $total = $this->database->query('SELECT COUNT(*) FROM {contact}')->fetchField();
  ...
  public function searchFormAlter(array &$form, FormStateInterface $form_state) {
    ...
    $form['advanced']['misc-fieldset']['first_name'] = array(
      '#type' => 'textfield',
      '#title' => t('First name'),
      '#description' => t('Search %field field for exact matches.', array('%field' => 'First name')),
      '#default_value' => isset($defaults['first_name']) ? $defaults['first_name'] : array(),
    );

    $form['advanced']['misc-fieldset']['gender'] = array(
      '#type' => 'select',
      '#title' => t('Gender'),
      '#options' => array(
        '' => t('Any'),
      ...
      '#default_value' => isset($defaults['gender']) ? $defaults['gender'] : array(),
    );
  ...
  public function buildSearchUrlQuery(FormStateInterface $form_state) {
    ...
    if ($form_state->hasValue('first_name') && !empty(($value = trim($form_state->getValue('first_name'))))) {
      $filters[] = 'first_name:' . $value;
      $advanced = TRUE;
    }

    if ($form_state->hasValue('gender') && !empty(($value = trim($form_state->getValue('gender'))))) {
